...
import { svg as MotionSvg } from "motion/react-m";
...
export const HeartsText: React.FC<HeartsTextProps> = ({
  text,
  ...
  const [sparkles, setSparkles] = useState<Heart[]>([]);

  useEffect(() => {
    const generateStar = (): Heart => {
    ...
    const initializeStars = () => {
      const newSparkles = Array.from({ length: heartsCount }, generateStar);
...
const Heart: React.FC<Heart> = ({ id, x, y, color, delay, scale }) => {
  return (
